...
def main ():
    ...
    if not args.predict:
        if not os.path.exists( os.path.join(outputPath,"trainData20x.pt")):
        ...
        if not os.path.exists( os.path.join(outputPath,"valData20x.pt")):
        ...
        predictionData = True
    if not os.path.exists( os.path.join(outputPath,"testData20x.pt")):
...
def multiprocess_collectTiles (libraryfile, featureVectorsPath, predictionData, max_cpu, stain_norm, trainData):
    ...
    total_slides = len(availableSlides)
...
def collectDownsampledTiles(currentSamples, lib, featureVectors, predictionData, stain_norm, trainData):
    ...
    currentTrainData20x['processed_slides'] = 0
...
# If image does not exist, create it
        ...
        currentTrainData20x['processed_slides'] +=1
