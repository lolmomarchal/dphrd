...
def inference(modelNumber, reps, t, loader, model):
    ...
    with torch.no_grad():
        ...
        for i, (input, target,softLabel, slide_ids) in tqdm.tqdm(enumerate(loader), total=len(loader), desc="[INFERENCE]"):
            ...
            logits, hrd_score_pred, raw_features = model(input)
            output = F.softmax(logits, dim=1)
            allFeatures.append(raw_features.cpu().numpy())
            batch_start = i * args.batch_size
            batch_end = batch_start + input.size(0)
            probs[batch_start:batch_end] = output.detach()[:, 1].cpu()
    allFeatures = np.concatenate(allFeatures, axis=0)
