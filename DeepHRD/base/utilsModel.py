...
def runMultiGpuInference (i, iModels, pythonVersion, outputPath, modelPath, batch_size, dropoutRate, resolution, workers, BN_reps):
	...
	for currentModel in iModels:
		...
		if resolution == '5x':
			...
			if os.path.exists(os.path.join(outputPath, "m" + str(currentModel+1), "predictions_" + resolution + ".csv")):
				continue
...
def multiResolution (outputPath, nModels, dropoutRate, threshold):
	...
	print(mat20x.columns)
	finalMat['target'] = list(mat5x['target'])
...
def combinePredictions (resolution, outputPath, nModels, dropoutRate):
	...
	final_path = os.path.join(outputPath, "predictions_" + resolution + "_n" + str(nModels) + "_models_" + str(dropoutRate) + ".csv")
	print(final_path)
	finalPredictions.to_csv(final_path)
import numpy as np

def groupPercentileTilesTesting(groups, data, percentile=90, min_k=1):
	"""
    Groups tiles per slide by percentile threshold instead of fixed top-K.

    Parameters:
        groups      : array-like
                      Slide indices per tile
        data        : array-like
                      Tile probabilities
        percentile  : float
                      Percentile threshold (e.g. 90)
        min_k       : int
                      Minimum number of tiles to keep per slide

    Returns:
        1. Tile indices (global indices)
        2. Corresponding slide indices
	...
	for g in np.unique(groups):
		mask = groups == g
		slide_probs = data[mask]
		slide_idx = np.where(mask)[0]

		if len(slide_probs) == 0:
			continue

		thresh = np.percentile(slide_probs, percentile)
		keep = slide_probs >= thresh

		# safety: ensure at least min_k tiles
		if keep.sum() < min_k:
			topk = np.argsort(slide_probs)[-min_k:]
			keep = np.zeros_like(slide_probs, dtype=bool)
			keep[topk] = True

		selected_idx.extend(slide_idx[keep])
		selected_groups.extend(groups[slide_idx[keep]])
		selected_probs.extend(slide_probs[keep])

	return selected_idx, selected_groups, selected_probs
...
class MILdataset(data.Dataset):
	...
	def preselect_epoch_slides(self, sampling_mode='dampened_combined'):
		...
		selected_slide_set = Counter(shuffled_original_slide_ids)
		...
		for i in range(len(self.grid)):
			...
			if original_slide_id in selected_slide_set:
				...
				for _ in range(selected_slide_set[original_slide_id]):
