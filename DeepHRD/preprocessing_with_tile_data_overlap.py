...
def training_slide_range_to_images(start_ind, end_ind, train_images):
    ...
    for slide_num in range(start_ind, end_ind + 1):
        try:
            ...
            count += 1 # Important to still increment count to not desync
...
def apply_filters_to_image_range(start_ind, end_ind, save):
	...
	for slide_num in range(start_ind, end_ind + 1):
		try:
			...
			print(f"--- !!! An error occurred while filtering slide #{slide_num} !!! ---")
			print(f"Error Type: {type(e).__name__}, Message: {e}")
			print(str(slide_num).zfill(3), file=skippedSamps, flush=True)
...
def multiprocess_training_slides_to_images(numProcessors=None):
	...
	check_path = os.path.join(BASE_DIR, "slideNumberToSampleName.txt")
	if os.path.exists(check_path):
		print(f"\n[VERIFICATION] Successfully created {check_path}\n")
	else:
		print(f"\n[VERIFICATION ERROR] FAILED to create {check_path}\n")
